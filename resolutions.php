...
if(isset($_POST['add'])){
    ...
    $chk = $conn->query("SELECT * FROM upload_res WHERE name = '$name' ")->rowCount();
    ...
    if($chk){
        ...
        while($c == 0){
            ...
            $chk2 = $conn->query("SELECT * FROM upload_res WHERE name = '$tname' ")->rowCount();
            if($chk2 == 0){
                $c = 1;
                $name = $tname;
            }
    ...
    $move =  move_uploaded_file($temp,"upload_res/".$fname);

    if($move){
        $query = $conn->query("INSERT INTO upload_res(name,fname) VALUES('$name','$fname')");
        if($query){
...
$query = $conn->prepare("SELECT * FROM upload_res ORDER BY id DESC LIMIT :limit OFFSET :offset");
...
<style>
    body{
        background: rgb(116,223,223);
background: linear-gradient(90deg, rgba(116,223,223,1) 0%, rgba(102,240,102,1) 63%, rgba(150,210,222,1) 100%);
    }
                    ...
                    <tbody id="dataTable">
                        ...
                        <td style="text-align: center;"><?php echo $row['reference_number']; ?></td>
                            ...
                            <td style="text-align: center;">
                                <a href="view_res.php?filename=<?php echo $row['id'];?>&f=<?php echo $row['fname'] ?>"><?php echo $row['name'] ?></a>
                            </td>
